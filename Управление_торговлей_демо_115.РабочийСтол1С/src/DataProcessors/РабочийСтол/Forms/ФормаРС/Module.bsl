...
Функция ПолучитьКартинкуОбъекта(НаименованиеОбъекта)
	...
	Если МассивСтрок.Количество() > 0 Тогда
		...
		Если СтрокаДанных.ВидОбъекта = "Справочник" Тогда
			Возврат ПоместитьВоВременноеХранилище(БиблиотекаКартинок.Справочник);
		ИначеЕсли СтрокаДанных.ВидОбъекта = "Документ" Тогда
			Возврат ПоместитьВоВременноеХранилище(БиблиотекаКартинок.Документ);
		ИначеЕсли СтрокаДанных.ВидОбъекта = "Отчет" Тогда
			Возврат ПоместитьВоВременноеХранилище(БиблиотекаКартинок.Отчет);
		ИначеЕсли СтрокаДанных.ВидОбъекта = "Обработка" Тогда
			Возврат ПоместитьВоВременноеХранилище(БиблиотекаКартинок.Обработка);		
		КонецЕсли;
	КонецЕсли;
...
Процедура КомандаСохранитьНастройки(Команда)
  ОткрытьФорму("Обработка.РабочийСтол.Форма.ФормаСохраненияНастройки",Новый Структура("ТекПользователь,АдресНастройки", ТекущийПользователь,АдресНастроек()))
КонецПроцедуры
...
Процедура КомандаЗагрузитьИзИзбранногоНаСервере()
	...
	Для Каждого Элемент Из Избранное Цикл
		Если СтрНайти(Элемент.НавигационнаяСсылка,"?ref=") <> 0 
		Или СтрНайти(Элемент.НавигационнаяСсылка,"list") = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		СтруктураДанныхОбъекта.Представление  = Обработки.РабочийСтол.ПредставлениеОбъекта(СтруктураДанныхОбъекта.Видобъекта,СтруктураДанныхОбъекта.ИмяОбъекта );
